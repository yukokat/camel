...
| *operation* (producer) | The operation to do in case the user don't want to send only a message. There are 4 enums and the value can be one of: send-batch-message, delete-message, list-queues, purge-queue |  | Sqs2Operations
...
The AWS 2 Simple Queue Service (SQS) endpoint is configured using URI syntax:
