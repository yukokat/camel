...
The AWS Key Management Service (KMS) component supports 12 options, which are listed below.
...
| *operation* (producer) | *Required* The operation to perform. There are 6 enums and the value can be one of: list-keys, create-key, disable-key, schedule-key-deletion, describe-key, enable-key |  | KMSOperations
